...
static AVL_Nodo* avl_nodo_insertar(AVL_Nodo* raiz, void* dato,
  ...
  if (raiz == NULL) // insertamos el nuevo elemento
    ...
    if(avl_nodo_factor_balance(raiz) == 2){
      //Casos 1 o 2
      if(avl_nodo_factor_balance(raiz->der) == -1){ // caso DI
        raiz->der = avl_nodo_rotacion_simple_der(raiz->der);
      }
      //Caso DD
      raiz->der = avl_nodo_rotacion_simple_izq(raiz->der);
    }
    raiz->altura = 1 + avl_nodo_max_altura_hijos(raiz);
...
int avl_nodos_nnodos(AVL_Nodo* raiz){
  if(raiz == NULL) return 0;
  else return 1 + avl_nodos_nnodos(raiz->izq) + avl_nodos_nnodos(raiz->der);
}

int avl_nnodos(AVL arbol){
  return avl_nodos_nnodos(arbol->raiz);
}

void destr_aux(void* dato){
  (void*)dato;
  return;
}
...
AVL_Nodo* avl_nodo_balancear(AVL_Nodo* raiz){
  ...
  if(factor == -2){//desbalance II o ID
...
AVL_Nodo* avl_nodo_eliminar(AVL_Nodo* raiz, FuncionDestructora destr, FuncionComparadora comp, void* dato){
  if(raiz == NULL) return raiz;
  if(comp(raiz->dato, dato) < 0) return avl_nodo_eliminar(raiz->der, destr, comp, dato);
  if(comp(raiz->dato, dato) > 0) return avl_nodo_eliminar(raiz->izq, destr, comp, dato);
  ...
  if(comp(raiz->dato, dato) == 0){
    ...
    if(raiz->izq != NULL && raiz->der != NULL){
      ...
      raiz->der = avl_nodo_eliminar(raiz->der, destr_aux, comp, dato);
    }
