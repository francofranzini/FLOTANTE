...
AVL_Nodo* avl_nodo_eliminar(AVL_Nodo* raiz, FuncionComparadora c, FuncionDestructora d, void* dato){
  ...
  if(c(raiz->dato, dato) == 0){
    ...
    raiz->der = avl_nodo_eliminar(raiz->der, c, eliminar_dummy, raiz->dato);
  }
  raiz->altura = 1 + avl_nodo_max_altura_hijos(raiz);
...
void* avl_nodo_buscar(AVL_Nodo* raiz, FuncionComparadora c, void* dato){
  if(raiz == NULL) return NULL;
  if(c(raiz->dato, dato) < 0) return avl_nodo_buscar(raiz->der, c, dato);
  if(c(raiz->dato, dato) > 0) return avl_nodo_buscar(raiz->izq, c, dato);

  return raiz->dato;
}

void* avl_buscar(AVL arbol, void* dato){
  void* buscado = avl_nodo_buscar(arbol->raiz, arbol->comp, dato);
  return buscado;
}